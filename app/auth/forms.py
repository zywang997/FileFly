...
class RegisterForm(Form):
    ...
    invitation_code = StringField('邀请码: ', validators=[
        DataRequired(message='填写10位邀请码'), 
        Length(10, 10)])
    ...
    def validate_invitation_code(self, field):
        userobj = UserTable.query.filter_by(invitation_code=field.data).first()
        if not userobj or userobj.invitation_times <= 0:
