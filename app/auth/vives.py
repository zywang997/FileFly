...
def register():
    form = RegisterForm()
    if request.method == 'POST' and form.validate_on_submit():
        invitor_user = UserTable.query.filter_by(invitation_code=form.invitation_code.data).first()
        if not invitor_user or invitor_user.invitation_times <= 0:
            flash('验证码无效')
            return redirect(url_for('auth.register'))
        invitor_user.invitation_times -= 1
        db.session.add(invitor_user)
