...
from . import main
from .. import db
from ..models import FileTable, RecycleTable, ShareTable
...
@main.route('/popular/', methods=['GET', 'POST'])
def popular():
    common_share = db.session.query(ShareTable).filter_by(share_pass='').all()
    common_file_ids = [cf.file_id for cf in common_share]
    fileobj = db.session.query(FileTable).filter_by(is_share=True, is_recycle=False).all()
    fileobj = [f for f in fileobj if f.file_id in common_file_ids]
    return render_template('index.html', fileobj=fileobj, isPopular=True)
...
def index():
    ...
    return render_template('index.html', fileobj=fileobj, isPopular=False)
